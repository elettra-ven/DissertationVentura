...
Il filo conduttore 
che attraversa tutti i capitoli è il modo in cui la ricerca farmaceutica si è trasformata da un 
processo empirico basato su \textit{trial and error} a un 
problema computazionale di ottimizzazione in spazi ad alta dimensionalità, 
affrontabile attraverso strumenti di apprendimento 
automatico.
...
stereochimiche particolari alle singole molecole. Questa progressione sempre più specifica e dettagliata non è unicamente legata ad aspetti tecnici, ma riflette una crescente comprensione di quali informazioni 
chimiche siano effettivamente \textbf{rilevanti per la predizione}.

Il capitolo sulla DTI e DTA ha mostrato come la formalizzazione 
del problema, attraverso la funzione $f(d,t) \rightarrow y$, abbia 
permesso di declinare la ricerca farmacologica in due task 
...
architetture di Deep Learning, rispecchia la crescente 
...
%\sidenote{\\Riassunto troppo lungo?}
...
rilevante. Sebbene molti algoritmi eccellano in scenare "warm", in cui le molecole del Test-Set somigliano a quelle dell'addestramento, la predizione di interazioni per farmaci o target \textbf{mai 
...
acquisita durante il pre-training, ma la questione rimane 
...
L'\textbf{interpretabilità} dei modelli deep learning 
rappresenta infine un requisito necessario per 
...
delle predizioni, ma rimangono processi non investigabili e difficili da seguire nella loro esecuzione (per questo vengono definiti "Black Box").
...
Le sfide descritte non hanno impedito ai modelli di intelligenza 
artificiale di produrre risultati concreti e validati 
clinicamente. Tre casi documentati illustrano come i 
metodi trattati in questo elaborato abbiano già cambiato 
la pratica della \textit{drug discovery}.

\subsection{Baricitinib per COVID-19}

Nel 2020, il team di BenevolentAI ha pubblicato 
su \textit{The Lancet} un'analisi che illustra come un 
...
già approvato per il trattamento dell'artrite reumatoide. 
...
\end{itemize}
Il farmaco ha 
successivamente ricevuto l'autorizzazione d'uso 
...
e ad alto impatto dei metodi di predizione DTI 
discussi in questo elaborato.

\subsection{Halicin: un antibiotico scoperto dal Deep 
Learning}
...
Il modello adottato è \textit{\textbf{Chemprop}}, una \textbf{D-MPNN} 
(\textit{Directed Message Passing Neural Network}), 
una variante delle GNN, descritte nei capitoli precedenti, con lo scopo di prevedere l'attività antibatterica 
a partire dalla struttura chimica del composto, espressa sotto forma di grafo.

Il modello traduce la rappresentazione in un vettore tramite \textit{message passing} basato sui legami chimici. Durante questo processo le caratteristiche tra atomi adiacenti vengono aggregate iterativamente per costruire una comprensione della chimica locale della molecola. 
...
Successivamente è avvenuta la fase di \textit{training}, svolto per il task di classificazione DTI su molecole in grado di inibile la crescita di \textit{E. coli}. Vengono coinvolte nello screening 2.560 molecole e vengono identificate come \textit{hit} 120 composti attivi (5.14\%).
...
Vengono individuate le 99 candidate con punteggi più alti, che vengono testate sperimentalmente. Di queste, 51 si confermano veri positivi.
...
\subsection{Rentosertib: il primo farmaco end-to-end 
generato dall'AI}
...
La generazione dell'inibitore specifico di TNIK è avvenuta tramite la piattaforma di chimica generativa \textbf{Chiemistry42}, che ha messo in parallelo 30 modelli con gli stessi punteggi e strutture condivisi, in modo da permettere l'esplorazione dello spazio chimico in modo collaborativo e non ridondante \cite{renSmallmoleculeTNIKInhibitor2025}.

Il sito di legame selezionato come luogo dell'interazione bersaglio è il 
\textbf{sito di legame dell'ATP di TNIK}.
La generazione è stata vincolata dalle seguenti ipotesi 
\textbf{farmacoforica} \cite{renSmallmoleculeTNIKInhibitor2025}: 
\begin{enumerate}
    \item Le strutture 
    candidate devono formare un legame idrogeno con 
    l'NH di \textbf{Cys108} nella regione hinge della 
    chinasi.
    \item I centri idrofobici devono essere posizionati nello spazio adiacente al gatekeeper \textbf{Met105}.
\end{enumerate}
...
per l'inibizione competitiva dell'ATP. 

Il flusso di lavoro integra approcci 
\textit{ligand-based} e \textit{structure-based}: 
le informazioni sui ligandi noti e la struttura 
cristallina del target guidano congiuntamente la 
generazione di nuove strutture molecolari.
Il \textbf{reinforcement learning} è utilizzato per 
ottimizzare iterativamente le molecole generate 
attraverso cicli di feedback durante lo screening 
virtuale, premiando strutture con profili farmacologici 
favorevoli. Modelli di ML aggiuntivi filtrano e 
classificano i composti candidati in base a novità 
strutturale, proprietà chimico-fisiche e accessibilità 
sintetica, riducendo drasticamente il numero di 
molecole da sintetizzare fisicamente \cite{renSmallmoleculeTNIKInhibitor2025}.

\section{Riflessioni Finali}

Le success stories documentate condividono una 
caratteristica comune:

In ciascun caso, i metodi 
computazionali non hanno sostituito la validazione 
sperimentale, ma l'hanno resa più mirata, riducendo 
drasticamente lo spazio di ricerca da esplorare in 
laboratorio. Il contributo fondamentale del 
ML e del DL alla pipeline farmaceutica non si ferma ad eliminare 
l'incertezza dei dati biologici, ma concentra le risorse 
sperimentali sui candidati più promettenti.

I casi di studio descritti suggeriscono che i progressi 
futuri più significativi non deriveranno 
dall'affinamento di singole architetture, ma 
dall'integrazione di rappresentazioni \textit{machine readable} in modelli 
multimodali capaci di affrontare i problemi DTI e DTA
nella loro complessità reale, e dall'unione tra selezione computazionale e validazione sperimentale. 
